...
$alarm_time=$_POST["time"];

$module_data=$module_mess . "+time=" . $alarm_time;
...
$zapros2 = mysql_query("DELETE FROM Signal WHERE obtaine_ip='$module_ip'");
        mysql_close($dbcnx);
// Отправляем пакет для обновления лаблицы relay на модуль с адресом module_ip

$module_data="update_Signal";
       ...
       $sock = socket_create(AF_INET, SOCK_DGRAM, SOL_UDP);
        ...
        socket_close($sock);
sleep(1);
header("Location: /cgi-bin/alarm_display.php?obtaine_ip=$module_ip");
?>
