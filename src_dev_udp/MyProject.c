...
/*********** RAM variables *************************/
unsigned char   myMacAddr[6] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x01} ;   // my MAC address
...
void interrupt() {
  if (PIR1.TMR1IF) {
    PIE1.TMR1IE = 0;       // Запрещение прерывания переполнения
    PIR1.TMR1IF = 0;     // Сброс флага переполнения TMR1
    TMR1H = 0x80;          // Начальные значения таймера TMR1
    Test_time = ~Test_time;
    asm CLRWDT;
    //==========Действие с интервалом в 1 секунду ===============
    ...
    //===========================================================
    PIE1.TMR1IE = 1;       // Разрешение прерывания переполнения
  }
}
void ReadEEPROM(void) {
    // ==== Чтение из памяти========
       Chip_Select = 0;                       // Select EEPROM chip
    Delay_ms(1);
    SPI1_Write(0x03);                      // Send read mode via SPI
    SPI1_Write(0x00);                      // Send high byte via SPI
    SPI1_Write(0x00);                      // Send low byte via SPI
  ...
  Chip_Select = 1;                       // DESelect EEPROM chip
    Delay_ms(1);
}
void WriteEEPROM(void){
                      ...
                      Chip_Select = 0;                       // Select EEPROM chip
                     Delay_ms(3);
                     SPI1_Write(0x06);                      // Send WREN mode via SPI
                     Delay_ms(3);
                     Chip_Select = 1;                       // DESelect EEPROM chip
                     Delay_ms(3);
                      ...
                      Chip_Select = 0;                       // Select EEPROM chip
                     Delay_ms(3);
                     SPI1_Write(0x01);                      // Send STATUS mode via SPI
                     SPI1_Write(0x00);                      // Send BP0,BP1 mode via SPI
                     Delay_ms(3);
                     Chip_Select = 1;                       // DESelect EEPROM chip
                     Delay_ms(3);
                     ...
                     Chip_Select = 0;                       // Select EEPROM chip
                     Delay_ms(5);
                     SPI1_Write(0x02);                      // Send WRITE mode via SPI
                     SPI1_Write(0x00);                      // Send high byte via SPI
                     SPI1_Write(0x00);                      // Send low byte via SPI
                     ...
                     SPI1_Write(0x02);                      // Send WRITE mode via SPI
                     SPI1_Write(0x00);                      // Send high byte via SPI
                     SPI1_Write(0x80);                      // Send low byte via SPI
                     ...
                     Delay_ms(3);
                     SPI1_Write(0x02);                      // Send WRITE mode via SPI
                     SPI1_Write(0x01);                      // Send high byte via SPI
                     SPI1_Write(0x00);                      // Send low byte via SPI
...
void main() {
    ...
    Relay6_time=0;
    obtaine_packet=0;
    INTCON = 0;            // Обнуление регистра управления прерываниями
    PIE1 = 0;              //
    PIE2 = 0;
//================ Управление прерываниями TMR1 ================================
    PIR1.TMR1IF = 0;       // Сброс TMR1IF flag bit
    TMR1H = 0x80;          // Начальные значения таймера TMR1
    TMR1L = 0x00;
    T1CON.RD16 = 0;        // Обработка как 8 битного регистра
    T1CON.T1RUN = 1;       // Источник - осциллятор
    T1CON.TMR1CS = 1;      // Timer1 подключение к внешнему кварцу
    T1CON.T1OSCEN = 1;     // Подключение цепи резонатора
    T1CON.T1CKPS1 = T1CON.T1CKPS0 = 0; // Подключение предделителя 1:1
    PIE1.TMR1IE = 1;       // Разрешение прерывания переполнения
    INTCON = 0xC0;         // Enable interrupt (bits GIE and PEIE)
    T1CON.TMR1ON = 1;      // Включение таймера TMR1 "on"
